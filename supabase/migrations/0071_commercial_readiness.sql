-- =============================================================================
-- 0071 — Conversão Comercial: prontos para apresentação (H3.15)
-- -----------------------------------------------------------------------------
-- Quando o Planner envia o caso à Conversão Comercial (Fase 3 → 4), o sistema
-- verifica se o cliente JÁ tem uma apresentação comercial FUTURA agendada
-- (normalmente marcada ainda na Conversão Clínica, Fase 2):
--   * SEMPRE avisa Consultor + Assistente Comercial que o plano está pronto —
--     dizendo a data da apresentação, ou "sem apresentação agendada".
--   * Se NÃO houver apresentação agendada: dispara um aviso FORTE à recepção
--     ("agende agora") e avisa Gerente de Unidade + Coordenador Clínico, para o
--     caso não travar.
-- Além disso, a recepção só é chamada para "agendar apresentação comercial"
-- quando de fato falta o agendamento (antes, era chamada sempre).
--
-- Recria public.move_client_phase (era da 0014) mudando apenas o ramo da
-- Conversão Comercial e a dica de agendamento. Idempotente (create or replace).
-- =============================================================================
...
declare
  ...
  v_presentation_at timestamptz; -- H3.15: apresentação comercial futura, se houver
begin
  ...
  if p_new_phase = 'planning_center' then
    ...
    -- H3.15: o caso só anda se houver uma apresentação comercial marcada. Busca
    -- a próxima apresentação FUTURA ainda válida (agendada/confirmada).
    ...
    -- Consultor + Assistente Comercial: plano pronto (com a data, se agendada).
    insert into public.notifications (user_id, clinic_id, title, body, link)
    select distinct ucr.user_id, v_clinic, 'Caso pronto para apresentação comercial',
      v_body || case
    ...
    if v_presentation_at is null then
      -- Aviso FORTE à recepção: precisa marcar a apresentação agora.
      insert into public.notifications (user_id, clinic_id, title, body, link)
      ...
      -- Gerente e Coordenador ficam sabendo do gargalo.
      insert into public.notifications (user_id, clinic_id, title, body, link)
      select distinct ucr.user_id, v_clinic,
  ...
  -- the client pre-filled. H3.15: a Conversão Comercial saiu daqui (o pedido de
  -- agendamento agora só dispara quando falta a apresentação, no ramo acima).
