...
| EV-112 | O print passou a conseguir mostrar uma caixa de seleção aberta | `capturarTela` (`contagem`/`aoContar`) + botão **Esperar 5 s** em `anexos.tsx` e na barra flutuante | ✅ preso por teste (`captura-de-tela.test.ts`, 2 testes): a contagem sai inteira (5…0) e o painel só sai da frente DEPOIS dela — a régua foi provada invertendo a ordem de propósito, e ela reprovou |
| EV-113 | Manter a caixa aberta sozinha não era possível — e a espera é o contorno, não um remendo | `getDisplayMedia` exige gesto do usuário e mostra o aviso de permissão | ✅ "Não identificado no código analisado" que permita evitar: o fechamento é do navegador (clique + troca de foco do aviso), fora do alcance da página; a tela e o manual dizem isso em vez de prometer o impossível |
