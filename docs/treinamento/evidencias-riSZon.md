...
| EV-069 | Admin Principal: só ele dá/tira Admin; o acesso de qualquer Admin só ele altera; ninguém troca o posto pela tela; matriz segue aberta a qualquer Admin | `0262` (gatilhos `proteger_admins`) + `src/lib/admins.ts` (8 testes) + `acesso-actions.ts` (`bloqueioDeHierarquia`, `definirAdmin`) | ✅ provado no banco do treino com dois Admins logados (16 checagens; estado restaurado) e na tela da produção como o dono (bloco Admin Principal; Tornar Admin na ficha de outra pessoa) |
| EV-070 | Guia "Este é o riSZon Treino" sempre no Início do treino (testar os limites, sempre disponível, LGPD, equipe vem do real) e ausente no sistema real | `src/components/como-usar-o-treino.tsx` + `(app)/page.tsx` | ✅ conferido logado no treino (v0.256.0) |
| EV-071 | Boas-vindas uma vez no primeiro login do sistema real; quem já entrou foi marcado pela 0263 | `src/components/boas-vindas.tsx` + `boas-vindas-actions.ts` + `0263` | ⏳ código no ar; **ninguém viu ainda** (os 2 perfis já estavam marcados) |
| EV-072 | Início com só as 3 últimas entregas + "Ver todas"; /novidades com busca (sem acento, linha a linha), filtro por tipo/ano e páginas de 10 | `changelog.ts` (`filtrarNovidades`, `paginarNovidades`) + 11 testes + `(app)/novidades/page.tsx` | ✅ conferido logado no treino (18 checagens): 3 no Início, 10 por página, busca "agenda" = 5 entregas (bate com a fonte), sem acento OK, sem resultado avisa, página 999 cai na última |
| EV-073 | Bloco Administração recolhido por padrão, abre num clique e sozinho em /admin | `app-sidebar.tsx` (cookie `risarte_admin_aberto`) | ✅ conferido: Início sem os 11 links e com `aria-expanded=false`; /admin/clinicas com os links |
| EV-074 | Relato do treino aparece na lista do sistema real com selo e abre na MESMA tela (conversa e anexos) | `src/lib/relatos-do-treino.ts` + `problemas/page.tsx` + `[codigo]/page.tsx` | ✅ ponta a ponta em 20/09: relato criado no treino apareceu (OC-00048), abriu na produção e foi apagado no fim |
| EV-075 | Admin responde do sistema real um relato do treino; a resposta fica no banco do treino; pela tela ninguém escolhe o autor | `0264` (`answer_system_report_como`) + `problemas/actions.ts` | ✅ provado: resposta gravada com `answered_by` = id do Admin no treino; chamada anônima recusada com NOT_ALLOWED |
| EV-076 | Régua de quem vê os relatos do treino é reaplicada no código (chave de serviço não tem RLS): Admin tudo, demais só os seus | `relatos-do-treino.ts` (`carregarRelatosDoTreino`, `carregarRelatoDoTreinoPorCodigo`) | ⚠️ conferido por leitura de código; falta provar logado como não-Admin |
| EV-077 | Painel consolidado: as mesmas regras da 0258 em `montarPainel`, com mediana sobre as linhas juntas | `painel-de-relatos.ts` + `painel-consolidado.test.ts` (17 testes) + `painel-dados.ts` | ✅ conferido contra o banco: 8 relatos no treino / 8 no painel (os dois) / 8 (só o treino); produção sem relatos mostra a tela vazia |
| EV-078 | Modo portal também relata e acompanha (/problemas e o relato abrem; boia na barra; unidade vem da ficha) | `auth.ts` (`PORTAL_PATHS`) + `problemas/page.tsx` + `topbar.tsx` + `problemas/actions.ts` | ✅ provado na produção com login NÃO-Admin em modo portal (antes: 307) |
| EV-079 | Cada pessoa vê no sistema real os relatos que ELA fez no treino, e não os de outros | `relatos-do-treino.ts` (régua da RLS reaplicada no código) | ✅ provado logado: vê OC próprio, 404 no de outra pessoa, painel barrado; relatos de teste apagados |
| EV-080 | Boas-vindas nos 3 primeiros acessos, no máximo 1 por dia | `0265` + `deveMostrarBoasVindas` (6 testes) | ✅ provado na produção mexendo só no contador do dono e restaurando: 3→não aparece; 0→aparece; vista hoje→não repete; ontem→volta |
| EV-081 | Textos automáticos (boas-vindas nas 2 versões + guia do treino) visíveis em Administração → Orientações, lidos da FONTE | `textos-automaticos.ts` + `orientacoes/textos-do-sistema.tsx` + 8 testes | ✅ conferido na produção: bloco presente, as duas versões, aviso de LGPD e o "quando aparece" |
| EV-082 | Mensagem pronta com os dados de acesso (endereço, login, senha provisória e TODAS as unidades com a função) | `mensagem-de-acesso.ts` (13 testes) + `enviar-acesso.tsx` | ✅ regras por teste; ⏳ falta ver na tela com um acesso recém-criado |
| EV-083 | Perfil mostra o próprio cadastro (foto, código, unidade, função, regime, contato, endereço), só leitura | `perfil/meu-cadastro.tsx` | ✅ conferido logado na produção (RIS-000005): bloco presente, com unidade, função, código, endereço e o aviso de só leitura |
| EV-084 | Lista mostra a FUNÇÃO PREVISTA do cadastro quando ainda não há função de acesso | `risartanos/dados.ts` + `page.tsx` | ⏳ **não observado no ar**: hoje todas as fichas da produção já têm função de acesso. Conferir no próximo Risartano cadastrado sem acesso |
| EV-085 | Só o Admin Master força a passagem de fase; as demais funções movem pelos ATOS do fluxo | `0266` + `journey.ts` (26 testes) + `kanban-board.tsx` / `journey-section.tsx` | ✅ provado no treino com logins reais: recepção, consultor e coordenador recusados (NOT_ALLOWED) nas passagens que saíram da matriz; os atos passaram; Admin forçou 7→3 |
| EV-086 | A auditoria diz se a passagem foi trabalho ou foi mão (`forcado`) | `0266` (`audit_logs.details.forcado`) | ✅ provado no treino nos dois sentidos: Admin forçando → `forcado: true`; ato do Coordenador → `forcado: false` |
| EV-087 | Coordenador conclui a reavaliação (6→7) e Planner devolve ao Coordenador (3→2 / 3→6) com motivo obrigatório | `jornada/actions.ts` + `clinical-tools.tsx` + `devolver-ao-coordenador.tsx` | ✅ regras provadas no treino (6→7 pelo coordenador); ⏳ falta ver os dois botões na tela |
| EV-088 | Consultor continua devolvendo ao Planejamento (4→3) — e só pelo ato, não pelo movimento cru | `0266` (`return_commercial_to_planning` marca `risarte.ato`) | ✅ provado no treino: pela função, a fase foi para o Planejamento; chamando `move_client_phase` direto, NOT_ALLOWED |
| EV-089 | Responder/encerrar um relato funciona para o Admin Master (defeito da 0264, corrigido na 0267) | `0267` + `answer_system_report` / `answer_system_report_como` | ✅ provado no treino, 9 checagens: responde pela tela, grava o autor certo, encerrar sem resposta continua recusado, e a porta do servidor não abre para usuário logado nem para quem não está logado |
| EV-090 | "Ir até a tela do problema" também na página de Problemas, levando o que já foi escrito | `levar-relato.ts` (3 testes) + `anexos.tsx` + `formulario.tsx` + `report-nav-item.tsx` | ✅ regras por teste; ⏳ falta ver na tela |
| EV-091 | Modo escuro: o navegador passou a desenhar os controles nativos no escuro (lista do select, calendário, rolagem) | `globals.css` (`color-scheme: dark` em `.dark`, `light` no claro) | ✅ medido no navegador com o sistema rodando: `getComputedStyle(:root).colorScheme = "dark"` |
| EV-092 | Agenda legível no escuro: contraste do texto do cartão saiu de 1.06 para 5.9–11.8 | `week-grid.tsx` (STATUS_STYLES com par escuro) | ✅ medido pixel a pixel no navegador (canvas), tema escuro, os 4 status; 1.0 = cores idênticas, e o mínimo recomendado para texto é 4.5 |
| EV-093 | O mesmo defeito procurado no sistema inteiro e preso por teste | `tema-escuro.test.ts` (2 testes) + 20 correções | ✅ 25 pontos com letra herdada sobre mancha clara corrigidos; 226 casos legítimos (letra escura declarada) NÃO são acusados; a régua foi provada quebrando a agenda de propósito — e a primeira versão dela não disparou, o que foi corrigido antes de valer |
| EV-094 | No agendamento, o cliente é procurado digitando (nome, código ou CPF), em toda a unidade | `escolher-cliente.tsx` + `buscarClientesParaAgendar` (reusa `search_clients`, 0251) | ✅ conferido no treino com o sistema rodando: "maria" achou por nome, "CBE-00004" por código, "345345" por CPF, cliente de OUTRA unidade recusado, escolha carregou a fase e o resto do formulário, e "trocar de cliente" não quebra |
| EV-095 | A lista de clientes da agenda era cortada em 300 nomes, sem aviso | `agenda/page.tsx` (limite passou a 50, declarado como "só o começo"; a pré-seleção por aviso deixou de depender dela) | ✅ lido no código antes da correção (`.limit(300)` + `clients.some(...)` na pré-seleção) |
| EV-096 | A gravação da consulta começa ao chamar o paciente (avaliação/reavaliação) e para ao concluir o atendimento | `gravacao-da-consulta.tsx` (barra) + gatilhos em `attendance-panel.tsx` + `atendimentoAindaAberto` | ✅ provado no treino com o sistema rodando: gravação iniciada, faixa com tempo correndo, **atravessou a troca de tela** (avaliação → prontuários, 00:12 seguindo), parada manual gravou 414 KB na ficha, ligada à rodada de avaliação |
| EV-097 | A dispensa do consentimento é estreita: só áudio, só nas fases 2 e 6 | `dispensaConsentimento` (`gravacao.ts`) + `gravacao.test.ts` (7 testes) + guarda em `recordClinicalMedia` | ✅ provado por teste caso a caso (foto/exame/vídeo/documento continuam exigindo; áudio fora da avaliação também; fase desconhecida exige). Decisão do dono com o jurídico, 21/09/2026, registrada no CLAUDE.md §6 |
| EV-098 | Gravar não é gravar escondido: faixa visível com o tempo, durante toda a consulta | `gravacao-da-consulta.tsx` (faixa fixa no rodapé, `role="status"`) | ✅ visto no treino: "Gravando a consulta · 00:06 — TESTE… — Parar e salvar" |
| EV-099 | A faixa de gravação sai do caminho: nasce no canto, move-se e encolhe — sem nunca sumir | `gravacao-da-consulta.tsx` + `limitarNaJanela` (4 testes) | ✅ conferido no treino com o sistema rodando: nasce no canto inferior direito (448 px, antes ocupava a largura toda); encolhe para 182 px mantendo ponto vermelho e tempo; arrastada de (1064,664) para (250,150); lugar e tamanho guardados no navegador; **gravação seguiu correndo (00:45) após trocar de tela** e salvou 848 KB na ficha |
| EV-100 | A apresentação comercial online não depende mais do horário nem dos dias da unidade | `0268` + `checkAgendaRules` + `getNextAvailableSlots` | ✅ provado no treino com o sistema rodando: apresentação marcada para **domingo 27/09 às 21:00** (a unidade não abre domingo e fecha às 18h), e a lista de horários passou a ir até 21:45 — a jornada do consultor, não a da unidade |
| EV-101 | A mudança é estreita: o presencial continua preso à agenda da unidade | mesma tela, mesmo dia | ✅ contraprova no mesmo domingo com tipo Retorno: *"A unidade não atende neste dia da semana."* |
| EV-102 | Jornada online em cascata — rede como padrão, exceção por consultor | `resolveOnlineAgenda` (7 testes) + `/admin/agenda-comercial` | ✅ regras por teste, incluindo a exceção PARCIAL (quem só muda o horário continua seguindo os dias da rede) e a lista de dias vazia caindo para a rede; padrão da rede gravado pela migração (08h–20h, seg–sáb) |
| EV-103 | O funil comercial move o atendimento: "acontecendo agora" → em atendimento; "apresentado/cancelado/perdido" → concluído | `0269` (`commercial_set_stage` chamando `update_attendance`) | ✅ provado no treino com dois logins reais: recepção fez o check-in (em espera) → consultor moveu o cartão → atendimento virou `in_service` → marcou apresentado → atendimento `done` e agendamento `completed` |
| EV-104 | O cartão do comercial mostra o que a recepção fez, com o tempo correndo | `estadoNaRecepcao` (6 testes) + `presentation-tracker.tsx` + janela de 4h na consulta | ✅ regras por teste (em espera com minutos, em atendimento contando da chamada, hora futura não vira tempo negativo); ⏳ falta ver na tela |
| EV-105 | O Consultor Comercial deixou de ver o menu Atendimento | `0269` (apaga a linha da matriz) + padrão em `permissions.ts` | ✅ a linha existia na matriz dos DOIS bancos (medido antes) — sem apagá-la, mudar o padrão do código não teria efeito nenhum |
| EV-106 | Em /problemas dá para escolher vários relatos e responder todos de uma vez | `responderVariosProblemas` + `barra-do-lote.tsx` | ✅ provado no treino pelo caminho do banco (3 relatos, mesma resposta em cada conversa) e **pela tela** (2 relatos escolhidos → resposta, situação e versão gravadas nos dois) |
| EV-107 | O briefing do conjunto junta N relatos num texto só, com as instruções uma vez | `montarBriefingDoConjunto` (7 testes) | ✅ conferido na tela com 3 relatos reais do treino: cabeçalho com códigos, partes do sistema, telas e quem relatou; um bloco por relato; "O QUE EU PRECISO" aparecendo uma única vez |
| EV-108 | O que não foi feito no lote volta dito, relato por relato | `ResultadoEmLote` + diálogo de resultado | ✅ regra provada no banco: repetir sem nada novo devolve NOTHING_TO_SAVE, que na tela vira "nada mudava neste relato" |
| EV-109 | Pular o "Chamar" deixa de travar o atendimento: o primeiro ato clínico chama o paciente | `0270` (`start_clinical_attendance`) + 4 ações clínicas | ✅ provado no treino com dois logins reais (recepção + coordenador), no caminho exato do relato OC-00075: check-in → escrever na ficha sem chamar → atendimento virou `in_service` com `called_by` do coordenador |
| EV-110 | O fim da avaliação encerra o atendimento do dia | `0270` (`finish_clinical_attendance`) em `sendToPlanningCenter` e `concluirReavaliacao` | ✅ provado no treino: o atendimento ficou `done` com hora registrada |
| EV-111 | A chamada automática é estreita: só quem está EM ESPERA, e nunca derruba o ato clínico | `0270` + posição da chamada depois da escrita | ✅ escrever de novo devolve `sem_espera` (não mexe em nada); quem não fez check-in não é chamado; e a função devolve o motivo em vez de levantar erro, para a anamnese/foto ser salva de qualquer jeito |
| EV-112 | A contagem antes da foto existe e a tela continua visível durante ela | `capturarTela` (`contagem`/`aoContar`) + `captura-de-tela.test.ts` | ✅ preso por teste: a contagem sai inteira (8…0) e o painel só sai da frente DEPOIS dela — provado invertendo a ordem de propósito, e a régua reprovou |
| EV-113 | A contagem sozinha NÃO resolvia: lista de seleção aberta não existe na foto da aba | teste do dono em 23/09/2026 ("fiz o teste mas não está capturando ainda caixa de seleção") | ✅ medido no uso real, não suposto. A causa: a captura de ABA copia o desenho da página, e a lista aberta é janela do sistema operacional desenhada por cima. **A primeira entrega estava errada e foi refeita no mesmo dia** |
| EV-114 | A captura com tempo passou a pedir a TELA INTEIRA, que é onde a lista existe | `origem: "tela-inteira"` (`displaySurface: "monitor"`, `preferCurrentTab: false`) | ✅ preso por teste (o pedido ao navegador é conferido campo a campo) e provado trocando "monitor" por "browser": a régua reprovou |
| EV-115 | Escolher "aba" na janela do navegador deixou de falhar em silêncio | `superficie` devolvida de `getSettings().displaySurface` + `avisarCaptura` | ✅ preso por teste (a superfície escolhida volta para quem chamou); na tela vira aviso de 12 s dizendo que o print saiu sem a lista e o que fazer — sem ele, o sintoma seria "o sistema não funciona" |
| EV-116 | Deixou de ser um interruptor que precisava ser ligado antes | botão **Capturar com tempo** (antes: interruptor "Esperar 5 s") | ✅ o próprio dono capturou sem perceber que precisava ligar o interruptor ("está muito confuso") — opção que depende de lembrar de ligar é opção que não existe |
| EV-117 | O sistema real abre em cada unidade pela função que a pessoa tem nela; uma missão por função vale em todas as unidades dessa função, e funções diferentes liberam cada uma sozinha | `0276` (`system_access_by_clinic`) | ✅ provado no treino com um Risartano real (recepção em A e B, gerência em C), numa transação desfeita: certificar recepção abriu A e B e deixou C fechada |
| EV-118 | Coletiva: a unidade com turma de novatos em andamento só abre quando o grupo inteiro cumprir | `0276` (lateral `grupo`) | ✅ provado no treino: com escopo individual a turma pendente não segurou B; com coletiva, B ficou em *aguardando o grupo* e abriu quando o colega cumpriu |
| EV-119 | Quem já trabalhava continua trabalhando (liberado antes do portão); função sem metas não barra; porta fechada na ficha fecha todas | `0276` (registro inicial + ordem do CASE) | ✅ provado no treino: 25 linhas *anterior* registradas; sem metas → aberta; porta fechada → fechada em todas, com certificado e tudo; mudar de função na unidade faz a liberação antiga deixar de valer |
| EV-120 | Liberar sem missão exige motivo; retirar marca e não apaga; só o Admin | `grant_clinic_access` / `revoke_clinic_access` | ✅ provado no treino: sem motivo recusado; retirada deixou a linha com `revoked_at`; não-Admin e chamada sem sessão recusados (NOT_ALLOWED) |
| EV-121 | Unidade fechada some da lista, e o Início explica qual e por quê; falha na conferência fecha, não abre | `auth.ts` + `acesso-por-unidade.ts` (8 testes) + `page.tsx` | ✅ regras por teste, provadas abrindo a guarda de falha de propósito (a régua reprovou); ⏳ falta ver o quadro na tela |
