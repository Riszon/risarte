...
| EV-058 | Liberar o treino cria o login no OUTRO banco, com função pelo nome da unidade; retirar bane | `src/lib/treino.ts` + `acesso-actions.ts` (`definirAmbiente`) | ✅ conferido ponta a ponta (retirar gravou `allowed:false`; liberar devolveu a senha provisória do treino) |
