...
| EV-061 | No treino, Risartanos, acessos, perfil e permissões só para consulta — para todos, inclusive o Admin | três barreiras: tela (`dados.ts` `alcanceDoUsuario`, `adminEdita`, `somenteLeitura`), ações (`SOMENTE_CONSULTA_NO_TREINO` em `risartanos/actions.ts`, `acesso-actions.ts`, `permissoes/actions.ts`, `perfil/actions.ts`) e banco (gatilho `mirror_read_only`, 0260) | ⏳ código e compilação; **falta provar no banco e na tela** depois da 0260 |
| EV-062 | Cada alteração na produção copia a pessoa/ficha inteira para o treino, depois da resposta; falha vira pendência com aviso | `src/lib/espelho-treino.ts` (`agendarEspelho`, `after`) + `mirror_state` (0260) | ⏳ falta provar ponta a ponta |
...
| EV-068 | Numeração RIS-… ocupada no treino por ficha local: a local é renomeada (-TREINO), nada se apaga | `espelho-treino.ts` (`liberarNumeracao`) + `espelho.ts` (`codigoAfastado`, 2 testes) | ⏳ testes puros; prova na próxima sincronização |
