...
| **PPR5** ✅ | **Motor de benefícios ligado**: camada única de programas (`src/lib/programs.ts`) escolhendo o melhor entre PPR+ e Empresarial; cobertura aplicada no orçamento e na venda direta; registro de uso com liberação por frequência; unidades, transferência e uso na rede (§15). Concluído no PPR5b: condições do plano (à vista, faixas do parcelado, parcela mínima e teto de parcelas) aparecem na negociação comercial, acima da regra da rede/unidade, com botão para aplicar o desconto. |
| **PPR6** ✅ | **Mensalidades e situação:** cobranças, suspensão por inadimplência, cancelamento e reativação (+ pontos do Riso+ Social). |
| **PPR7** ✅ | **Dashboard do PPR+** + **ranking das unidades** para a Franqueadora. Todo indicador é **clicável** e abre a lista por trás do número (mesmo padrão do Dashboard do Comercial, `ppr-drill.tsx`), com **chips de filtro** dentro do pop-up (titular × dependente, situação do plano, entrou × saiu); o crescimento tem **dois gráficos separados** — quantidade de planos e receita — porque misturar grandezas confunde a leitura. O painel tem **filtro de período** que vale para o movimento (entradas, saídas, limpezas); estoque (planos, beneficiários, receita) é sempre "hoje". O crescimento é lido como **saldo do período** (entrou − saiu) e o percentual só aparece com base ≥ 5 (`growthPercent`). |
